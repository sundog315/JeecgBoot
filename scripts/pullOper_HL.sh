    ...
    echo "$result_array" | grep -o '{[^}]*}' | while read -r script_obj; do
        ...
        if [ ! -f "$full_path" ] || [ "$current_version" != "$server_version" ]; then
            ...
            local content_response=$(curl -s -X GET \
                "${API_SCRIPT_URL}/getScriptContent?scriptPath=${full_path}&deviceModule=${device_type}")
...
handle_frp() {
    ...
    local custom_domains=$(echo "$param" | awk -F ',' '{print $6}')
    local multiplexer=$(echo "$param" | awk -F ',' '{print $7}')
    local frp_type=$(echo "$param" | awk -F ',' '{print $8}')

    # 更新FRP配置
    update_frp_config "$server_addr" "$server_port" "$token" "$ssh_port" "$http_port" "$custom_domains" "$multiplexer" "$frp_type"
...
update_frp_config() {
    ...
    if [ -n "$custom_domains" ]; then
        sed -i "/config conf 'CPE_[0-9A-Fa-f]\{6\}_ssh'/,/^config conf/ {s/option custom_domains '[^']*'/option custom_domains '$custom_domains'/}" "$temp_file"
        sed -i "/config conf 'CPE_[0-9A-Fa-f]\{6\}_http'/,/^config conf/ {s/option custom_domains '[^']*'/option custom_domains '$custom_domains'/}" "$temp_file"
    fi
    
    if [ -n "$multiplexer" ]; then
        sed -i "/config conf 'CPE_[0-9A-Fa-f]\{6\}_ssh'/,/^config conf/ {s/option multiplexer '[^']*'/option multiplexer '$multiplexer'/}" "$temp_file"
    fi
    
    if [ -n "$frp_type" ]; then
        sed -i "/config conf 'CPE_[0-9A-Fa-f]\{6\}_ssh'/,/^config conf/ {s/option type '[^']*'/option type '$frp_type'/}" "$temp_file"
    fi
    ...
    if [ -f "/etc/config/frpc" ]; then
        ...
        function process_block() {
            ...
            for (i=0; i<line_count; i++) {
                if (lines[i] ~ /^[ \t]*option custom_domains/) { split(lines[i], a, q); cd=a[2]; }
                if (lines[i] ~ /^[ \t]*option type/) { type_idx=i; }
            }
            ...
            if (cd == "") {
                ...
                if (name ~ /ssh$/) target_type = "tcpmux";
                else if (name ~ /http$/) target_type = "http";
            }
            
            if (target_type != "" && type_idx != -1) {
                sub(/option type \047[^\047]*\047/, "option type " q target_type q, lines[type_idx]);
            }
            
            for (i=0; i<line_count; i++) print lines[i];
        }
        ' /etc/config/frpc > /tmp/frpc_temp && mv /tmp/frpc_temp /etc/config/frpc
    fi
